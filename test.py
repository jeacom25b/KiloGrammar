...
def push_into(parser, matches, node, *args):
    assert(type(node) == Node)
    for arg in args:
        node.contents.append(arg)

def set_name(parser, matches, name_grp, *args):
    print(type(name_grp))
    assert(type(name_grp) == tuple)
    assert(len(name_grp) == len(args))
    for name, arg in zip(name_grp, args):
        arg.type = name

def is_name(parser, matches, name_grp, *args):
    print(type(name_grp))
    assert(type(name_grp) == tuple)
    assert(len(name_grp) == len(args))
    for name, arg in zip(name_grp, args):
        if not arg.type == name:
            return False
    return True

def reverse(parser, matches, node):
    assert(type(node) == Node)
    node.contents = list(reversed(node.contents))

def pop(parser, matches, n=1, index=-1):
    for _ in range(n):
        parser.stack.pop(index)

def pop_from(parser, matches, node, n=1, index=-1):
    assert(type(node) == Node)
    return node.contents.pop(index)
...
def look_ahead(parser, matches, *args):
    for i, names in enumerate(args):
        assert(type(names) == tuple)
        token_index = i + parser.token_ptr
        if token_index < len(parser.tokens):
            if not is_name(parser, matches, parser.tokens[token_index], names):
                return False
    return True

def Or(parser, matches, *args):
    for arg in args:
        if arg:
            return arg
    return False

def And(parser, matches, *args):
    for arg in args:
        if not(arg):
            return False
    return True
